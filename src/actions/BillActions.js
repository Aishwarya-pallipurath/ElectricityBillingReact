...
export const createBill=(bill,history)=>async dispatch=> {
    try {
        const res =await axios.post ("http://localhost:8081/createNewBill",bill)
...
export const getBills=()=>async dispatch=>{
    const res=await axios.get("http://localhost:8081/getAllBills");
      dispatch({
        type:GET_BILLS,
        payload:res.data
    })
...
export const getBill=(billId,history)=>async dispatch=>{
    const res=await axios.get(`http://localhost:8081/getBillById/${billId}`);
      dispatch({
        type:GET_BILL,
        payload:res.data
    })
...
export const deleteBill=(billId)=>async dispatch=>{
    if(window.confirm("Are you sure ? This will delete the bill and the data related to it")) {
        await axios.delete(`http://localhost:8081/deleteBillById/${billId}`);
        dispatch({
            type:DELETE_BILL,
            payload:billId
        })
